% This script shows an example fit to the spectra for an example electrode
%
...
%%
stims_plot = [39]; % plot this stimulus
...
plot(f,mean(data_base_boot),'k','LineWidth',1)
...
for k = 1:length(stims_plot)
    ...
    % plot fit without Gauss
    plot(f,10.^(resamp_parms(k,2)-resamp_parms(k,1)*log10(f)),':','Color',stims_color{k},'LineWidth',1)
    ...
    plot(f,mean(data_boot),'Color',stims_color{k})
end
...
print('-dpng','-r300',fullfile(dataDir,'derivatives','spectra','fit',...
    ['Figure8_fitSpectra_s' int2str(s) '_stim' int2str(stims_plot(1))]))
