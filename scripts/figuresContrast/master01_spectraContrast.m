...
subjects = {'19','24','1001'};
electrodes = {109,45,50}; % example electrodes per subject {109,45,50};
...
for s = 3%:length(subjects)
    ...
    stims_plot = [47 48 49 50 39 83];
    stims_color = {[1 1 0],[1 .7 0],[1 .3 0],[1 0 0],[.5 .1 .1],[0 .3 .9]};
...
%             [0 0 0])
    ...
    for k = 1:length(stims_plot)
        % get stimulus data
        data_fit = data_fft(stims==stims_plot(k),:); % stimuli
        plot(f,mean(data_fit,1),'Color',stims_color{k},'LineWidth',2)
    end
