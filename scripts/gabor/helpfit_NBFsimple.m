function [results,modelfit] = helpfit_NBFsimple(stimulus,params,ecog_vals,seeds)
%
...
%%%% NBF model
modelfun = @(pp,dd) pp(4) * std(reshape(reshape(dd,[],res*res) * gaufun1(pp),[],nrOrientations),[],2);
...
% fix x, y, sigma
boundsFix = [NaN NaN NaN   0;
          NaN NaN NaN  Inf];
bounds = [NaN NaN 0   NaN;
          NaN NaN Inf  NaN];
model = {{[]         boundsFix   modelfun} ...
         {@(ss) ss   bounds      @(ss) modelfun}};
